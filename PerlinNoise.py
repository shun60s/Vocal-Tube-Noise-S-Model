...
if __name__ == '__main__':
    ...
    # instance (1) for /fu/
    pn = PerlinNoise(speed_up_rate=70.0, tc=2.0)
    fu= pn.make(length = int( 120 * 0.001 / (1.0 / pn.sampling_rate)) )  # 120ms
    pn.plot_waveform('Waveform (accelerated velocity)')
    wavwrite( 'fu_noise_av.wav', pn.sampling_rate, ( fu * 2 ** 15).astype(np.int16))
    print ('save fu_noise_av.wav')
    
    
    # instance (2) for /s(u)/
    # In order to generate higher frequency noise, use upsampling method.
    upsampling_ratio=16
    pn = PerlinNoise(lattice_size=1024, speed_up_rate=30.0, tc=2.0, upsampling_ratio=upsampling_ratio)  # 持続時間が長い信号なので、格子サイズlattice sizeは大きめにする
    length = int( 150 * upsampling_ratio * 0.001 / (1.0 / pn.sampling_rate)) # 150ms 　オーバーサンプリングするので、そのupsampling_ratio倍　作成する
    y = pn.make(length)
    
    # donwsampling 
    yd=  signal.decimate(y,upsampling_ratio)
    wavwrite( 's_noise.wav', pn.sampling_rate , ( yd * 2 ** 15).astype(np.int16))
